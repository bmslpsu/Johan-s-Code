function find_focal_grid_WS_v1(calibration_loc,neck_loc,neck_loc_images)

%modified so it doesn't need any model and mask info
neck_loc_images=round(neck_loc_images); %needs to be rounded so that it has no decimal points (might not be needed but makes things simpler for now)
load([calibration_loc 'DLT.txt']); %loads the calib_par_cam_(1,2,3)
calib_par_cam_1=DLT(:,1);
    calib_par_cam_2=DLT(:,2);
    calib_par_cam_3=DLT(:,3);

    [U_grid, V_grid] = meshgrid(1:640,1:480);
    %for my apploication can use 1:end for length and width of image(WS)
    
    X_uv = [reshape(U_grid,1,640*480); reshape(V_grid,1,640*480); ones(1,640*480)]; %converts into a 3xsomething matrix containing the coordinates of each point in the image
...
%some rotations, need clarifications (WS)
    ...
    %creates an empty image to crop the actual image 
    zero_mat1=zeros(480,640);
    zero_mat2=zeros(480,640);
    zero_mat3=zeros(480,640);
    zero_mat1(neck_loc_images(1,2)-127:neck_loc_images(1,2)+128,neck_loc_images(1,1)-127:neck_loc_images(1,1)+128)=1;
    zero_mat2(neck_loc_images(2,2)-127:neck_loc_images(2,2)+128,neck_loc_images(2,1)-127:neck_loc_images(2,1)+128)=1;
    zero_mat3(neck_loc_images(3,2)-127:neck_loc_images(3,2)+128,neck_loc_images(3,1)-127:neck_loc_images(3,1)+128)=1;
    
    reshaped1=reshape(zero_mat1,1,640*480);
    reshaped2=reshape(zero_mat2,1,640*480);
    reshaped3=reshape(zero_mat3,1,640*480);

    P1 = XW_cam_1(1:3,:);
    P2 = XW_cam_2(1:3,:);
    P3 = XW_cam_3(1:3,:);
    
    P_11=P1(:,find(reshaped1==1));
    P_22=P2(:,find(reshaped2==1));
    P_33=P3(:,find(reshaped3==1));
    %P_vox coordinates of the voxels 
    %P_vox_neighbors coordinates :helps disregard certain voxels
    [vox_1_mat, vox_2_mat, vox_3_mat, P_vox, P_vox_neighbor] = calculate_focal_grid(e1,e2,e3,P_11,P_22,P_33,neck_loc);
